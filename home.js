...
async function cargarEventos(){
    await fetch ('https://amazing-events.herokuapp.com/api/events')
        ...
        console.log(dataApi);
            homeEventos = dataApi.events
            console.log(homeEventos);
...
function checkboxCreada(){
    var checkboxes = document.getElementById("checkboxes") 
                               // de home eventos mapeo las categorias y las guardo en la variable todaslascategorias
    var todaslasCategorias = homeEventos.map(eventos => eventos.category) //Recorro el array de paises y separo de este la propiedad continent
    ...
    var categorys = [...arrayData]  //guardo el dato obtenido cono anteri el metodor en la variable continents
    
    var inputCheckbox = ""
    categorys.forEach(category =>{ //Recorro el array continenets recibido como parametro y los creaa abajo
...
checkbox.forEach(check => check.addEventListener("click", (event)=>{
    ...
    if(verificado){  //Establezcon un condicional que verifica si la propiedad/atributo checked del elemento html, es true o false, es decir si esta tildado o no el checkbox
        checkboxElegida.push(event.target.value) //Si esta tildado lo empujo lo guardo dentro de la variable local declarada anteriormente
...
function filtrarArray(){
    ...
    if (checkboxElegida.length > 0 && textSearch !== ""){ // crea la 1ra condicion, el check y el buscador estan con contenido
        checkboxElegida.map(category => { //recorre el array y category 
            datas.push(...homeEventos.filter(carta => carta.name.toLowerCase().includes(textSearch.trim().toLowerCase()) && // la carta que coincida con el buscador, lo guarda en mi array datas 
        ...
        checkboxElegida.map(category => datas.push(...homeEventos.filter(carta => carta.category == category))) //esta buscando las categorias que coincidan
    }
    else if (checkboxElegida.length == 0 && textSearch !== ""){ // solo esta usando el buscador
        datas.push(...homeEventos.filter(cartas => cartas.name.toLowerCase().includes(textSearch.trim().toLowerCase()))) //esta buscando que coincida el buscador con el nombre de la carta           
    }

    else {datas.push(...homeEventos)} // no este seleccionado el check ni el buscador, entonces muestra todo los eventos
    // se cargo el array datas con los eventos que coinciden con alguna de estas cuatro condiciones
    ...
    // }
    // else{displayCardEvents(datas)//llama a imprimir las cartas
    // }
    displayCardEvents(datas)
}
filtrarArray()


function displayCardEvents(datas){
